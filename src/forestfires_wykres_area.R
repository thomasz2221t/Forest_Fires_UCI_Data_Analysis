...
#View(dane)
counts <- table(dane$month, dane$area )
...
teren <- stem #c(stjan,stfeb,stmar,stapr,stmay,stjun,stjul,staug,stsep,stoct,stnov,stdec)

macierz<-table( teren,miesiace)

#ramka = data.frame(miesiace, temperatury)

barplot(stem)
barplot( height =teren,names = miesiace, ylim = c(0,26), main="Sredni obszar pozaru w miesiacach",
